export class CanvasLocal {
    ...
    pintar(x1, y1, x2, y2, x3, y3, x4, y4, //son las coordenadas de los cuatro puntos que forman el polígono.
    color, strokeColor) {
    ...
    grafiBarra(x, y, alt, color, porcentaje) {
    ...
    paint() {
        ...
        for (let i = 0; i < totalBarras; i++) {
            ...
            this.graphics.fillText(`${Math.round(porcentaje)}%`, this.iX(x + (anchoBarra / 2) - 0.3), // Calculo la posición X del texto
            this.iY(-0.8) // Calculo la posición Y del texto
            );
            this.grafiBarra(x + anchoBarra / 2, // Posición X de la barra
